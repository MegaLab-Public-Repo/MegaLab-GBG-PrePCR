...
{
    public class Setup_Cytomat_Verify_Barcodes_Full : BioSero.GreenButtonGo.GBGScript
    {
        public void Run(Dictionary<String, Object> variables, RuntimeInfo runtimeInfo)
        {
            // This script analyzes the contents of the all plates of the hotel (samples and consumable and compares against the rules
            // Hotel 1 must only contain ELU plates.  ELU count == Minimum (Default 16)
            ...
            // A pass condition will have Error Text variable == Null

            //DONE: Verify if all ELU, ART1, ART2, CDNA are unique (ctrl+f "Distinct()")
            //DONE: Edit Minimum Qty for ART1, ART2, and CDNA variables to reflect default above (see Pre-PCR Recipe.json)
            //DONE: Add Zoning Logic to assign ART1 and ART2 to fit with Donald's logic.  Ensure paired ART1/ART2 plates are selected (under comment "Specify ART1/2 plates to run")

            // Get RegEx Values
            Regex eluRegex = new Regex((string)variables["Recipe.Sample Plate Validation Regex"]);
            ...
            Regex cdnaRegex = new Regex((string)variables["Recipe.Intermediate Plate Validation Regex"]);
             ...
             // Get Plate Count Values
            int eluPlateCountMinimum = Convert.ToInt32((double)variables["Recipe.ELU Plate Count Minimum"]);
            int eluSetPlateCount = Convert.ToInt32((double)variables["Recipe.ELU Set Plate Count"]);
            ...
            // Get all Plates in hotel 1 to determine if ELU
            var eluPlates = Database.GetAllRows("Cytomat_Hotels", new[] { "Hotel", "Barcode" })
                .Where(r => r[0] == "Hotel 1" && !string.IsNullOrWhiteSpace(r[1]));
        ...
        //Verify ELU matches regex and count ELU plates
        int eluPlateCount = 0;
                foreach (string[] plate in eluPlates)
                {
                    debug.Append(string.Format("Found ELU Barcode: {0} ({1}).\n", plate[1], plate[0])); //List of plates to debug
                    
                    if (!eluRegex.IsMatch(plate[1])) 
                        errorSb.Append(string.Format("Barcode has incorrect format: {0} ({1}).\n", plate[1], plate[0]));
                    else
                        eluPlateCount++;
                }
                            
                //Verify ELU count is equal to minimum
                if (eluPlateCount != eluPlateCountMinimum) 
                    errorSb.Append(string.Format("Unexpected ELU plate count: {0}. Expected {1}. Verify that only ELU plates are contained in Hotel 1.\n", eluPlateCount, eluPlateCountMinimum));
    
                //Verify ELU plate barcodes are all unique
                if (eluPlates.Distinct().Count() != eluPlates.Count())
                    errorSb.Append(string.Format("Found duplicate ELU plate barcodes: {0}. All barcodes must be unique.\n", eluPlates.GroupBy(r => r[1]).OrderByDescending(bc => bc.Count()).Select(grp => grp.Key).First()));
   
                int art1PlateCount = 0;
                int art2PlateCount = 0;
                int artPlateCountTotal = 0;
                StringBuilder art1Worklist = new StringBuilder();
                StringBuilder art2Worklist = new StringBuilder();
                ...
                foreach (string[] plate in artPlates)
                {
                    ...
                    if (isArt1)
                    {
                        ...
                        matchingPlate = artPlates.FirstOrDefault(p => art2Regex.IsMatch(p[1]) && p[1].Split('-')[0] == barcodeSegments[0]);
                            
                        //Specify ART1/2 plates to run
                        if (art1PlateCount <= (artPlateCountMinimum / 2) && matchingPlate != null)
                       {
                            art1Worklist.Append(string.Format("{0},", plate[1]));
                            art2Worklist.Append(string.Format("{0},", matchingPlate[1]));

                            if (art1PlateCount == artPlateCountMinimum)
                            {
                                art1Worklist.Remove(art1Worklist.Length - 1, 1);
                                art2Worklist.Remove(art2Worklist.Length - 1, 1);
                            }
                        }
                    }
                    ...
                    matchingPlate = artPlates.FirstOrDefault(p => art1Regex.IsMatch(p[1]) && p[1].Split('-')[0] == barcodeSegments[0]);
                   }
                
                if (matchingPlate == null)
                    errorSb.Append(string.Format("No plate barcode was found matching '{0}'\n", plate[1]));
               else
                    artPlateCountTotal++;
            }
                ...
                if (artPlateCountTotal < artPlateCountMinimum)
                    errorSb.Append(string.Format("Unexpected ART plate count: {0} total ART1 and ART 2 plates. Expected {1}. Verify that only ART plates are contained in Hotels 3 through 7.\n", artPlateCountTotal, artPlateCountMinimum));
                ...
                debug.Append(string.Format("ELU plate count: {0}, min: {1}.  Hotel 2 plate count: {2}, min: {3}.  ART total plate count: {4}, min: {5}.  cDNA plate count: {6}, min: {7}", eluPlateCount, eluPlateCountMinimum, hotel2PlateCount, hotel2PlateCountRequired, artPlateCountTotal, artPlateCountMinimum, cdnaPlateCount, cdnaPlateCountMinimum));
                
                variables["Process.ART1 Worklist"] = art1Worklist.ToString();
                variables["Process.ART2 Worklist"] = art2Worklist.ToString();
